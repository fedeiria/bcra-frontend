...
export class HistoryChart implements OnChanges {
  ...
  */
  formatPeriod(periodoRaw: string | number): string {
  ...
  getSituationClass(situation: number): string {
    return CREDIT_SITUATUION_CONFIG[situation]?.class || '';
  }

  private renderChart(): void {
    if (!this.chartCanvas || !this.historyData || this.historyData.length === 0) return;
    ...
    const labels = orderedData.map(item => this.formatPeriod(item.periodo));
  ...
  async exportToExcel(): Promise<void> {
    ...
    const metadata = [
      { label: 'Razón Social / Denominación:', value: this.clientName || 'N/A' },
      { label: 'CUIT:', value: this.clientCuit || 'N/A' },
    ...
    this.historyData.forEach((item, index) => {
      ...
      row.values = [
        this.formatPeriod(item.periodo),
        item.deudaTotal, // Pasamos el número limpio (Opción 2 sin multiplicar por 1000)
        item.situacion,
    ...
    let nextAvailableRow = startRow + this.historyData.length + 3;

    if (this.chartCanvas && this.chartCanvas.nativeElement) {
      try {
        const canvas = this.chartCanvas.nativeElement;
        // Extract the chart as a Base64-encoded PNG image with maximum quality
        const chartImageBase64 = canvas.toDataURL('image/png', 1.0);

        // Add the image to the workbook and get its ID for referencing
        const imageId = workbook.addImage({
          base64: chartImageBase64,
          extension: 'png',
        });

        // Insert a title for the chart above the image
        ...
        // Add the image to the worksheet, positioning it below the last row of data and spanning across multiple columns for better visibility. The dimensions are set to fit well within an A4 page when printed.
        worksheet.addImage(imageId, {
          tl: { col: 0, row: nextAvailableRow },     // Top-Left
          ext: { width: 550, height: 260 }          // Dimensions in pixels (adjust as needed for better fit)
        });
    ...
    const fileName = `Reporte_Crediticio_${this.clientCuit || 'Cliente'}.xlsx`;
  ...
  exportToPdf(): void {
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);
    doc.text(`Razon Social / Denominación: ${this.clientName || 'N/A'}`, 14, 55);
    doc.text(`CUIT: ${this.clientCuit || 'N/A'}`, 14, 61);
    ...
    // Table Body
    const tableBody = this.historyData.map(item => [
      this.formatPeriod(item.periodo),
      new Intl.NumberFormat('es-AR', { style: 'currency', currency: 'ARS', maximumFractionDigits: 0 }).format(item.deudaTotal),
      `Situación ${item.situacion} - ${this.getSituationLabel(item.situacion)}`
    ]);
    ...
    if (this.chartCanvas && this.chartCanvas.nativeElement) {
      try {
        const canvas = this.chartCanvas.nativeElement;
        // Converts the canvas to a Base64-encoded PNG image with maximum quality
        const chartImageBase64 = canvas.toDataURL('image/png', 1.0);

        // Get the final Y position after the table to determine where to place the chart
    ...
    doc.save(`Reporte_Crediticio_${this.clientCuit || 'Cliente'}.pdf`);
  }
}
